%% optimized 9 point stencile
%% instead of adding a frquency*identity
%% the identity matrix becomes complciated
close all;
clear; 
...
%% Create the omega^2 component
Dws_f = c*sign*speye(M,M); %M fully determines the matricial size;
...
coef = (1-c-4*d)/4;
Dws_f(linear_ind_rtx) = coef*sign;
Dws_f(linear_ind_ltx) = coef*sign;
Dws_f(linear_ind_rbx) = coef*sign;
Dws_f(linear_ind_lbx) = coef*sign;
...
for w = ['x', 'y']
    ...
    Dws_f(linear_ind_r0x) = d*sign;
    Dws_f(linear_ind_l0x) = d*sign;
