...
Ce = [speye(M) -Dzf Dyf; Dzf speye(M) -Dxf; -Dyf Dxf speye(M)];
Ch = [speye(M) -Dzb Dyb; Dzb speye(M) -Dxb; -Dyb Dxb speye(M)];
...
figure;
EzC = (log(abs(EzC)));
slice((EzC), xslices, xslices, xslices)
colormap hsv
colorbar
xlabel('x direction')
ylabel('y direction')
