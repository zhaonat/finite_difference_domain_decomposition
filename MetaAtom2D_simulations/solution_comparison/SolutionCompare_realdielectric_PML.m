...
for k = 3:3
    ...
    Mz = zeros(N);
    ind_src = Npml + [SingleCellSize, SingleCellSize];%ceil(N/2);  
    ...
    [Aschur, bmod, App, Avvcell, Apvcell, Avpcell, bvcell, InvAvvStorage] = ...
        ParallelIterativeFourBlockSchurSingleSep_PartitionedPML(SymA, SymB, hpart, vpart,divx, divy,N,...
...
%     [xInt, flag4, iter4, error4] = NZqmr(SymA, SymB, tol, maxitun, []);
    ...
    %% Reconstruct Field Solutions
    Hz = reshape(x1, length(x1)^.5, length(x1)^.5);
    resctructuredTolSol = ...
    MultiCellSchurInteriorSolGeneral(x2, Avvcell, InvAvvStorage, ...
                            Avpcell, bvcell);  
    toc
    Hzrec = reshape(Q\resctructuredTolSol, length(resctructuredTolSol)^.5, ...
        length(resctructuredTolSol)^.5);
    
    
end
dir = 'D:\Nathan\Documents\StanfordYearOne\Fan Group\FDFDDocumentation\PaperFigures\SolutionCompare\Data'
save(strcat(dir, strcat('\SolutionComparisonRealDielectricPML_',num2str(k),'.mat')));
