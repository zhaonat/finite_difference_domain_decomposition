...
for numCells = 1:15
    ...
    Npml = 1*[5 5];  % [Nx_pml Ny_pml] need to deal with the special case where both are 0
    ...
    CellDimx = N(1)/divx;
    CellDimy = N(2)/divy;
    conditions = [conditions; eigs(A,1,'smallestabs')]
...
figName = strcat('Npml=',num2str(Npml(1)),'_wvlen=', num2str(wvlen));
figure; 
plot(systemSize,condData);
title(figName)
xlabel('system Size')
ylabel('eigenvalues')