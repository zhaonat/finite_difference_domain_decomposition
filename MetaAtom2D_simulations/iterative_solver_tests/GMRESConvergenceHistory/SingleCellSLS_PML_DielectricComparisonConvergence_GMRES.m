...
for epsilon = [1,12,12i]
    ...
    [xSchur, flag1, relres1, iter1, resvec1, iterates1] = ...
        gmres_iterates(Aschur, bmod,50, tol, 10*length(bmod), [],[],[],Sampling1);
    toc
    disp('full solve');
    tic
    [xTol, flag2, relres2, iter2, resvec2, iterates2] = ...
        gmres_iterates(A, b,50, tol, 10*length(b), [],[],[],Sampling2);
    toc
    ...
    savefig(sdf, strcat('GMRESNx=',num2str(N(1)),'wvlen=',num2str(wvlen),'_Soln+TotalConvHist_eps=',num2str(epsilon),'.fig'))
    saveas(sdf, strcat('GMRESNx=',num2str(N(1)),'wvlen=',num2str(wvlen),'_Soln+TotalConvHist_eps=',num2str(epsilon),'.png'))
    save(strcat('GMRESNpml=',num2str(Npml(1)),'_Nx=',num2str(N(1)),'wvlen=',...
